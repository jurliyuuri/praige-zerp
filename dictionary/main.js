...
const get_word = (dictionary, id, image_getter) => {
    ...
    const audio = get_audio_names(word)?.map(name => `<p class="word_info"><span class="bordered_info">音声</span><audio controls><source src="../audio/${name}.mp3" type="audio/mpeg">
    ...
    const linzi_transcription = (() => {
        ...
        for (const form of tr.forms) {
            ...
            const all_converted = Array.prototype.map.call(form, (l) => image_getter(l, ["SY", "meloviliju", "jv", "jv touch panel", "SY pua2 man1", "noborder", "border"], 30, false, "http://jurliyuuri.com/lin-marn"));
            let res = "";
...
const encode_syllable = (str) => {
    ...
    const encoded_vowel = vowel === "a" ? "00" :
        vowel === "ia" ? "01" :
            vowel === "ua" ? "02" :
                vowel === "ai" ? "03" :
                    vowel === "uai" ? "04" :
...
const encode_syllable_traditional2 = (str) => {
    const match = str.match(/^([pbmcsxztdnlkgh]?)([aeiouy]+[ptkmn]?)([12]?)$/);
    if (match === null) {
        alert("Warning: unexpected syllable `" + str + "` was encountered while sorting.");
        return "";
